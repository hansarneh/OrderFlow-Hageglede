...
async function testOngoingWMSAPI() {
  console.log('🔍 Testing Ongoing WMS API Structure (v5 - With Goods Owner ID)...\n');
  ...
  // Try different goods owner IDs (common values)
  const goodsOwnerIds = [1, 2, 3, 4, 5, 10, 100];
  
  // Test endpoints that might require goods owner ID
  const testEndpoints = [
    // Try with goods owner ID as query parameter
    '/orders',
    '/order',
    '/articles',
    '/article',
    ...
    '/goods-owner/{id}/orders',
    '/goods-owner/{id}/articles',
    '/goods-owner/{id}/inventory',
  ...
  for (const endpoint of testEndpoints) {
    for (const goodsOwnerId of goodsOwnerIds) {
      try {
        ...
        apiUrl = apiUrl.replace('{id}', goodsOwnerId);
        
        // Add goods owner ID as query parameter
        const separator = apiUrl.includes('?') ? '&' : '?';
        apiUrl = `${apiUrl}${separator}goodsOwnerId=${goodsOwnerId}`;
        ...
        if (response.ok) {
          ...
          const result = {
            endpoint: endpoint.replace('{id}', goodsOwnerId),
            goodsOwnerId,
          ...
          // If we found a working endpoint, don't test other goods owner IDs for this endpoint
          break;
        } else {
  ...
  for (const endpoint of postEndpoints) {
    for (const goodsOwnerId of goodsOwnerIds) {
      try {
        ...
        console.log(`Testing POST ${apiUrl} with goodsOwnerId: ${goodsOwnerId}`);
        ...
        if (response.ok) {
          ...
          console.log(`✅ POST ${endpoint} (goodsOwnerId: ${goodsOwnerId}) - Status: ${response.status}, Type: ${result.dataType}, Count: ${result.itemCount}`);
          results.push(result);
          
          // If we found a working endpoint, don't test other goods owner IDs for this endpoint
          break;
        } else {
          console.log(`❌ POST ${endpoint} (goodsOwnerId: ${goodsOwnerId}) - Status: ${response.status}, Error: ${response.statusText}`);
        }
      } catch (error) {
        console.log(`❌ POST ${endpoint} (goodsOwnerId: ${goodsOwnerId}) - Error: ${error.message}`);
      }
    }
  }
