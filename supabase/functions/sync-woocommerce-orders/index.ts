...
function validateAndNormalizeUrl(storeUrl: string): string {
  try {
    ...
    if (hostname === 'localhost' || 
      ...
      throw new Error('Local URLs (localhost, 127.0.0.1, private IP addresses) cannot be accessed by Supabase Edge Functions. Please use a publicly accessible domain name.');
    }
  ...
  try {
    ...
    if (error instanceof TypeError) {
      if (error.message.includes('Failed to fetch')) {
        throw new Error('Unable to connect to WooCommerce store. This usually means:\n1. The store URL is not publicly accessible\n2. The store is behind a firewall or VPN\n3. The domain name doesn\'t exist or is misconfigured\n\nPlease verify your store URL is correct and accessible from the public internet.');
      }
...
async function fetchWooCommerceOrdersPage(baseUrl: string, auth: string, page: number): Promise<{ orders: WooCommerceOrder[], hasMore: boolean }> {
  const apiUrl = `${baseUrl}/wp-json/wc/v3/orders`;
  
  console.log(`--- FETCHING PAGE ${page} ---`);
  
  try {
    // Fetch orders with specific statuses: processing and delvis-levert
    const pageUrl = `${apiUrl}?per_page=100&page=${page}&status=processing,delvis-levert`;
    
    console.log(`Page URL: ${pageUrl}`);
    console.log(`Requesting page ${page} with per_page=100...`);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!integration || !integration.credentials) {
      return new Response(
        JSON.stringify({ error: 'WooCommerce integration not configured. Please add your WooCommerce credentials in Settings → Integrations.' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (!credentials.storeUrl || !credentials.consumerKey || !credentials.consumerSecret) {
      return new Response(
        JSON.stringify({ error: 'Incomplete WooCommerce credentials. Please check your settings and ensure all fields are filled.' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    const maxPages = 2; // Reduced from 20 to 2 to prevent timeouts

    while (hasMoreOrders && page <= maxPages) {
      try {
        // Fetch one page of orders
        const { orders: pageOrders, hasMore } = await fetchWooCommerceOrdersPage(baseUrl, auth, page);
    ...
    const message = page > maxPages 
      ? `Incremental sync completed (limited to ${maxPages} pages): ${totalSyncedCount} orders synced, ${totalErrorCount} errors. Run sync again to continue if more orders exist.`
      : `Incremental sync completed: ${totalSyncedCount} orders synced, ${totalErrorCount} errors`;
    ...
    return new Response(
      JSON.stringify({ 
        ...
        dateRange: 'All dates (no date filter applied)',
        timestamp: new Date().toISOString()
      }),
      { 
        status: 200, 
